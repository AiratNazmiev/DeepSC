...
parser.add_argument('--vocab-file', default='DeepSC/data/europarl/vocab.json', type=str)
parser.add_argument('--checkpoint-path', default='DeepSC/checkpoints/deepsc-AWGN', type=str)
#parser.add_argument('--checkpoint-path', default='checkpoints_debug/deepsc-Rayleigh', type=str)
...
parser.add_argument('--mi', action=argparse.BooleanOptionalAction)
parser.add_argument('--mi-lambda', default=0.01, type=float)
...
def train(epoch, args, net, mi_net=None):
    ...
    for sents in pbar:
        ...
        if mi_net is not None:
            ...
            loss = train_step(net, sents, sents, 0.1, pad_idx,
                              optimizer, criterion, args.channel, mi_net, mi_lambda=args.mi_lambda)
            pbar.set_description(
    ...
    mi_log = np.mean(mi_log)
            
    return loss_log, mi_log
...
if __name__ == '__main__':
    ...
    if args.mi:
        mi_net = Mine(hidden_size=256).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        curr_loss_log, curr_mi_log = train(epoch, args, deepsc, mi_net)  # MI is added
        ...
        loss_log.append(curr_loss_log)
        mi_log.append(curr_mi_log)
        val_loss_log.append(avg_acc)
    
   
    np.save(args.checkpoint_path+'/loss_log', loss_log)
    np.save(args.checkpoint_path+'/mi_log', mi_log)
    np.save(args.checkpoint_path+'/val_loss_log', val_loss_log)
